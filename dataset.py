...
class MarkdownDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        code_inputs = self.tokenizer.encode_plus(
            self.dict_cellid_source[row['code']],
            None,
            add_special_tokens=True,
            max_length=128,
            padding='max_length',
        ...
        return torch.LongTensor(inputs['input_ids']), torch.LongTensor(inputs['attention_mask']), torch.LongTensor(code_inputs['input_ids']), torch.LongTensor(code_inputs['attention_mask']), torch.LongTensor([label])
...
class RowDataset(Dataset):
    ...
    def __init__(self, dict_cellid_source, rows):
        super().__init__()
        self.dict_cellid_source = dict_cellid_source
        self.tokenizer = AutoTokenizer.from_pretrained(BERT_MODEL_PATH)
        self.rows = rows
    ...
    def __getitem__(self, index):
        ...
        return torch.LongTensor(inputs['input_ids']), torch.LongTensor(inputs['attention_mask']), torch.LongTensor(id_to_label(row['mark']))
