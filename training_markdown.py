...
from helper import get_features_mark, get_features_new, validate_markdown, validate_sigmoid
...
train_fts = get_features_new(train_df, 'train')
val_fts = get_features_new(val_df, 'test')

ranks = []
for ft in train_fts:
    ranks.append(ft['pct_rank'])

# fixed bin size
bins = np.arange(-2.5, 2.5, 0.001)  # fixed bin size

# plt.xlim([min(ranks)-5, max(ranks)+5])

plt.hist(ranks, bins=bins, alpha=0.5)
